// client/src/pages/MentorPendingLeavesPage.jsx
import React, { useState, useEffect, useCallback } from 'react';
import leaveService from '../api/leaveService.js';
import Modal from '../components/Modal';
import '../components/Modal.css';

const downloadFile = (filename, content, contentType) => { // Helper
...
const MentorPendingLeavesPage = () => {
  ...
  const fetchPendingLeaves = useCallback(async (clearMessages = true) => {
    ...
    try {
      const data = await leaveService.getMentorPending();
      setPendingLeaves(data);
    } catch (err) {
      setError(err.message || 'Failed to fetch pending leave applications.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleApprove = async (leaveId) => {
    ...
    try {
      await leaveService.approveByMentor(leaveId);
      setActionMessage({ type: 'success', text: 'Leave approved successfully! Status changed to Pending HOD.' });
      fetchPendingLeaves(false);
    } catch (err) {
      setActionMessage({ type: 'error', text: err.message || 'Failed to approve leave.' });
    }
  };
  ...
  const submitRejectionFromModal = async () => {
    ...
    setActionMessage({ type: '', text: '' });
    try {
      await leaveService.rejectByMentor(currentLeaveIdForRejection, rejectionReasonText);
      setActionMessage({ type: 'success', text: 'Leave rejected successfully!' });
      setIsRejectionModalOpen(false);
      fetchPendingLeaves(false);
    } catch (err) {
      setRejectionModalError(err.message || 'Failed to reject leave.');
    } finally {
  ...
  if (loading && pendingLeaves.length === 0) return <div>Loading pending leave applications...</div>;
  if (error) return <div className="error-message">Error: {error}</div>;

  return (
    <div>
      <h2>Leaves Pending Your (Mentor) Approval</h2>
      {actionMessage.text && <p className={actionMessage.type === 'success' ? 'success-message' : 'error-message'}>{actionMessage.text}</p>}
      {pendingLeaves.length === 0 && !loading ? <p className="info-message">No leave applications pending your approval.</p> : (
        <table>
          <thead>
            <tr>
              <th>Mentee Name</th><th>Email</th><th>Start Date</th><th>End Date</th><th>Type</th><th>Reason</th><th>Submitted</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
        ...
        {selectedLeaveDetails && !detailLoading && (
          <>
            <dl className="detail-grid">
              {/* ... (Identical <dt><dd> structure as in MyLeavesPage for details) ... */}
      ...
      <Modal isOpen={isRejectionModalOpen} onClose={() => {setIsRejectionModalOpen(false); setRejectionModalError('');}} title="Provide Rejection Reason (Mentor)">
        <div>
...
export default MentorPendingLeavesPage;