// client/src/pages/HODPendingLeavesPage.jsx
...
const HODPendingLeavesPage = () => {
  ...
  const fetchPendingLeaves = useCallback(async (clearMessages = true) => {
    ...
    try {
      const data = await leaveService.getHODPending(); // <<< CHANGED
      setPendingLeaves(data);
    } catch (err) {
      setError(err.message || 'Failed to fetch HOD pending leave applications.'); // <<< CHANGED
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleApprove = async (leaveId) => {
    if (!window.confirm('Are you sure you want to approve this leave application (HOD)?')) return; // <<< CHANGED
    setActionMessage({ type: '', text: '' });
    try {
      await leaveService.approveByHOD(leaveId); // <<< CHANGED
      setActionMessage({ type: 'success', text: 'Leave approved by HOD! Status changed to Pending Dean.' }); // <<< CHANGED
      fetchPendingLeaves(false);
    } catch (err) {
      setActionMessage({ type: 'error', text: err.message || 'Failed to approve leave as HOD.' }); // <<< CHANGED
    }
  };
  ...
  const submitRejectionFromModal = async () => {
    ...
    setActionMessage({ type: '', text: '' });
    try {
      await leaveService.rejectByHOD(currentLeaveIdForRejection, rejectionReasonText); // <<< CHANGED
      setActionMessage({ type: 'success', text: 'Leave rejected by HOD!' }); // <<< CHANGED
      setIsRejectionModalOpen(false);
      fetchPendingLeaves(false);
    } catch (err) {
      setRejectionModalError(err.message || 'Failed to reject leave as HOD.'); // <<< CHANGED
    } finally {
  ...
  const handleDownloadLeave = () => { /* ... Same as Mentor Page ... */ 
  ...
  if (loading && pendingLeaves.length === 0) return <div>Loading HOD pending leave applications...</div>; // <<< CHANGED
  if (error) return <div className="error-message">Error: {error}</div>;

  return (
    <div>
      <h2>Leaves Pending Your (HOD) Approval</h2> {/* <<< CHANGED */}
      {actionMessage.text && <p className={actionMessage.type === 'success' ? 'success-message' : 'error-message'}>{actionMessage.text}</p>}
      {pendingLeaves.length === 0 && !loading ? <p className="info-message">No leave applications pending your HOD approval.</p> : ( // <<< CHANGED
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {pendingLeaves.map(leave => (
              <tr key={leave._id}>
                ...
                <td>
                  <button onClick={() => handleApprove(leave._id)} className="approve-button">Approve</button>
                  <button onClick={() => openRejectionModal(leave._id)} className="reject-button">Reject</button>
      ...
      <Modal isOpen={isDetailModalOpen} onClose={closeDetailModal} title="Leave Application Details">
         {/* ... Identical Modal Content as Mentor Page ... */}
      ...
      <Modal isOpen={isRejectionModalOpen} onClose={() => {setIsRejectionModalOpen(false); setRejectionModalError('');}} title="Provide Rejection Reason (HOD)"> {/* <<< CHANGED */}
        <div>
          {rejectionModalError && <p className="error-message" style={{fontSize: '0.9em', marginBottom: '10px'}}>{rejectionModalError}</p>}
          <textarea rows="4" style={{ width: '95%', display:'block', marginBottom: '10px', padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }} value={rejectionReasonText} onChange={(e) => { setRejectionReasonText(e.target.value); if (rejectionModalError) setRejectionModalError(''); }} placeholder="Enter reason for rejection..." required />
          <button onClick={submitRejectionFromModal} className="reject-button" disabled={isSubmittingRejection}>{isSubmittingRejection ? 'Submitting...' : 'Confirm Rejection'}</button>
...
export default HODPendingLeavesPage;